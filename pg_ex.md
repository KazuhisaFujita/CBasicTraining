...
__補足__ char型は整数型でもある．また，int，longなど環境によって使える桁数が変わる変数型がある．符号なし(正の値)の変数も作ることが可能である．その場合は方の前にunsignedと書く．
...
__補足__ C言語ではint main(void)と書く． C++ではint main()でもよい．引数が空の場合の挙動がCとC++では違うため(C++では引数が空の場合void)．
...
1. 次のソースコードを実行した場合に表示される値を予想し答えよ．
2.  実行した場合に表示される値を報告せよ．
3. 数値をいろいろ変え，出力される値の規則性を答えよ．たとえば四捨五入になるなど．
4. 次のソースコードを実行し表示される値を報告せよ．
5. このソースコードを実行すると正しい計算結果が出力される．リスト1とリスト2で結果が異なった理由を考え説明せよ．
...
__課題1.5 (発展)__ 任意の金額を入力し，その金額を500円玉で払うためには何枚必要かを求めるプログラムを作成せよ．
．
...
フローチャートとは物事の処理の流れ，特にアルゴリズムを図で表す際に用いられる手法である．世界中で用いられており，日本ではJIS規格により仕様が定められている．フローチャートは特にプログラミングの処理のやり方を記すために使用されるが，その他に仕事の手順などプログラミングとは関係ない場面でも用いられる．そのため，将来プログラミングの仕事をしない場合でも必要となる場面があるのでマスターしたほうがよい．
...
ループの例として，ループ処理を使た1からNまでの整数の和をプログラムを見てみよう，
